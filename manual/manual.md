...
Для это воспользуемся ранее установленной утилитой `packer`.
...
Файлы `centos.json` и `centos-packer-1.6.json` содержат пример описания произвольного образа,
`centos.json` для версий `packer<1.6`, `centos-packer-1.6.json` для версий `packer>=1.6`.
Полное описание можно найти в документации к `packer`. Обратим внимание на основные секции или ключи.
...
В секции `builders` задаем исходный образ, для создания своего в виде ссылки и контрольной суммы.
Именно эта секция различается в файлах `centos.json` и `centos-packer-1.6.json`.
Параметры подключения к создаваемой виртуальной машине.
...
```
packer build centos.json  # либо centos-packer-1.6.json в зависимости от версии packer
```
...
Если все в порядке, то машина будет запущена и загрузится с новым ядром. В данном примере это `5.7.11`.
...
В свежих версиях Linux, например Ubuntu 20.04, вы можете столкнуться с ошибкой при публикации пакета.
```SSL_connect returned=1 errno=0 state=error: no protocols available (OpenSSL::SSL::SSLError)```
Как ее обойти можно прочитать [здесь](https://stackoverflow.com/questions/62592042/unsupported-protocol-opensslsslsslerror-error-when-sending-mail-with-actio)
